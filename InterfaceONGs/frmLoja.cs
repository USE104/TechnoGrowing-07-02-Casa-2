...
namespace InterfaceONGs
{
    public partial class frmLoja : Form
    {
        ...
        // enviando infos para PesquisarProdutos
        public string nome = "";
        public string codigo = "";

        public frmLoja(string nome)
        {
            InitializeComponent();
            //this.nome = nome;
            //this.cbbCodigoFunc.Text = codigo;
            //txtnomeProd.Text = nome;
            //cbbCodigoFunc.Items.Add(codigo);
        }
        ...
        private void buscarcodProd()
        {
            ...
            while (dr.Read())
            {
                cbbcodLoja.Items.Add(dr.GetInt32(0));
            }
            Conexao.fecharConexao();
        }
        ...
        {
            MySqlCommand comm = new MySqlCommand();
            comm.CommandText = "select * from tbLoja where  codLoja like '%" + cbbcodLoja.Text + "%' order by codLoja";
            comm.CommandType = CommandType.Text;
            comm.Connection = Conexao.obterConexao();
        ...
        private void cbbcodLoja_SelectedIndexChanged(object sender, EventArgs e)
        {
            string indic = cbbcodLoja.SelectedItem.ToString();
            string indice = cbbcodLoja.Text.ToString();

            string pegainfs = indic;
            string pegacodProd = indice;
        }
        ...
        private void alterarProduto()
        {
            ...
            comm.CommandText = "update tbLoja set nomeProduto = @nomeProduto,Valor = @Valor,Quantidade = @Quantidade,Descricao =@Descricao,codFunc = @codFunc  where codLoja = " + cbbcodLoja.Text + ";";
        ...
        private void btnDeletar_Click(object sender, EventArgs e)
        {
            ...
            MySqlCommand comm = new MySqlCommand();
            comm.CommandText = "delete from tbLoja where Descricao = @Descricao";
            comm.CommandType = CommandType.Text;
            comm.Connection = Conexao.obterConexao();
